...
def main():
    ...
    runtime=resolve_runtime_paths(ROOT).outputs_dir;runtime.mkdir(parents=True,exist_ok=True);rs=rows();reports=[]
    for seed in (42,43,44):
        report=run(seed,rs,runtime);reports.append(report)
        if not report["pass_gate"]: break
    summary={"version":"v0.6.4","seeds":[{"seed":r["seed"],"steps_to_gate":r["steps_completed"] if r["pass_gate"] else None,"final":r["final"],"pass_gate":r["pass_gate"]} for r in reports],"all_three_passed":len(reports)==3 and all(r["pass_gate"] for r in reports),"full_training_allowed":len(reports)==3 and all(r["pass_gate"] for r in reports),"stopped_after_first_failed_seed":next((r["seed"] for r in reports if not r["pass_gate"]),None)};(runtime/"v064_sanity_summary.json").write_text(json.dumps(summary,ensure_ascii=False,indent=2)+"\n",encoding="utf8");print(json.dumps(summary,ensure_ascii=False,indent=2));assert summary["all_three_passed"]
