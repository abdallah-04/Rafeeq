...
const DIFF_COLORS: Record<string, { bg: string; text: string }> = {
  ...
  AUTISM:             { bg: '#F0FDF4', text: '#16A34A' },
  DYSLEXIA:           { bg: '#FFF1F2', text: '#E11D48' },
  DEVELOPMENTAL_DELAY:{ bg: '#FFF7ED', text: '#F97316' },
  SPEECH_DELAY:       { bg: '#EFF6FF', text: '#3B82F6' },
...
function EmptyState({ t }: { t: any }) {
  return (
    <View style={styles.emptyContainer}>
      ...
      <View style={styles.ghostCard} /><View style={styles.ghostCard} />
    ...
    <TouchableOpacity style={styles.studentCard} onPress={() => router.push(`/(school)/student/${student.id}` as any)} accessibilityRole="button">
      ...
      <View style={styles.studentInfo}>
        <View style={styles.topRow}>
          ...
          <View style={[styles.statusBadge, student.status === 'ACTIVE' ? styles.badgeGreen : styles.badgeOrange]}>
            <Text style={[styles.statusText, student.status === 'ACTIVE' ? styles.textGreen : styles.textOrange]}>
              {student.status === 'ACTIVE' ? t('students.active') : t('students.pending','Pending')}
            </Text>
          </View>
        </View>
        <View style={styles.tagsRow}>
          {student.level != null && <View style={styles.levelBadge}><Text style={styles.levelText}>{t('students.level')} {student.level}</Text></View>}
          {student.learningDifficulty && <View style={[styles.diffBadge, { backgroundColor: diffColor.bg }]}><Text style={[styles.diffText, { color: diffColor.text }]}>{diffKey}</Text></View>}
        </View>
        <View style={styles.progressBarBg}><View style={[styles.progressBarFill, { width: `${student.assessedLevel ? Math.min(student.assessedLevel*20,100) : 0}%` }]} /></View>
      </View>
    </TouchableOpacity>
  )
}
...
export default function StudentsScreen() {
  ...
  const load = useCallback(async () => {
    try { setStudents(await apiGetStudents()) }
  ...
  return (
    <SafeAreaView style={styles.safe}>
      ...
      <ScrollView contentContainerStyle={styles.container}
        ...
        {students.length === 0 ? <EmptyState t={t} /> : (
...
const styles = StyleSheet.create({
  ...
  studentName: { fontSize: 15, fontFamily: 'Lexend-Bold', fontWeight: '700', color: colors.textPrimary },
  statusBadge: { borderRadius: 99, paddingHorizontal: 10, paddingVertical: 3 },
  badgeGreen: { backgroundColor: '#E6F9F0' }, badgeOrange: { backgroundColor: '#FFF7ED' },
  statusText: { fontSize: 11, fontFamily: 'Lexend-SemiBold', fontWeight: '600' },
  textGreen: { color: '#22C55E' }, textOrange: { color: '#F97316' },
